...
function PlatformCard({ step, idx, scrollYProgress }: CardProps) {
  ...
  const segStart = idx === 0 ? 0 : (idx - 1) / totalSegments;
  const segEnd = idx === 0 ? 0 : idx / totalSegments;
      ...
      <div className="w-full h-full flex flex-col justify-between px-10 lg:px-20 py-12 lg:py-16">
        ...
        <div className="flex-1 flex flex-col lg:flex-row items-center gap-8 lg:gap-16 my-8">
          ...
          >
            ...
            <div className="grid grid-cols-2 gap-x-10 gap-y-3 border-t border-slate-900/10 pt-6 max-w-md">
              {step.bullets.map((bullet, bIdx) => (
                <div
                  key={bIdx}
                  className="text-[10.5px] font-mono font-bold tracking-wider text-slate-900 flex items-center gap-2"
                >
                  <span className="text-slate-900/30 text-sm font-light leading-none">
                    /
                  </span>
                  {bullet.toUpperCase()}
                </div>
          ...
          >
            ...
            >
              <Image
                ...
                sizes="(max-width: 360px) 30vw, 55vw"
              />
            </motion.div>
          </motion.div>
        </div>
